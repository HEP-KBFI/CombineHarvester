...
p=subprocess.Popen(["mkdir %s" %outPath], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
cmd = 'text2workspace.py %s -o %s/workspace.root' %(inputPath, outPath)
target_ws = 'workspace'
phoniesToAdd.append(target_ws)
lines.append("%s: " %target_ws)
lines.append("\t%s" %cmd)
lines.append("\t")

target = 'GOF_data'
cmd = "combine -M GoodnessOfFit --algo=saturated %s/workspace.root -n _%s_%s_%s_data -m 125" %(outPath, channel, era, signal_type)
phoniesToAdd.append(target)
lines.append("%s:%s " %(target, target_ws))
lines.append("\t%s" %cmd)
lines.append("\t")
...
for run in range(1,nrun+1):
   ...
   cmd = "combine -M GoodnessOfFit --algo=saturated %s/workspace.root -n %s -m 125 -t %s --toysFreq --seed %s" %(outPath, tree_name, ntoys_perrun, run)
   phoniesToAdd.append(target)
   lines.append("%s:%s " %(target, target_ws))
   lines.append("\t%s" %cmd)
   lines.append("\t")
...
toys = ROOT.TH1D("toys","toys",50,minx,maxx)
data = ROOT.TH1D("data","data",50,minx,maxx)
toys.SetLineWidth(3)
...
legend.AddEntry(None, "p-value: %0.3f" %pvalue,'')
legend.Draw()
c.SaveAs("%s/gof_%s_%s_%s.pdf"%(outPath, channel, era, signal_type))
c.SaveAs("%s/gof_%s_%s_%s.png"%(outPath, channel, era, signal_type))
