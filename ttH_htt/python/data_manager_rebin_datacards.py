...
def getHist_tobeFlat(hist_dict, histSource) :
    for key in hist_dict.keys() :
        ...
        if 'LBN_%s' %key in histSource or 'LBN_GGF_%s' %key in histSource or 'LBN_VBF_%s' %key in histSource:
            print 'found key====== ', key
...
def rebinRegular_bbww(
    ...
    notconsider = ['hdamp', '_QCDbased', 'GluonMove', '_erdON', '_mtop', 'width']
    ...
    for nn,nbins in enumerate(nbin) :
        ...
        for nkey, key in enumerate(file.GetListOfKeys()):
            ...
            if 'hdamp' in h2.GetName() or 'QCDbased' in h2.GetName() or 'GluonMove' in h2.GetName() or '_erdON' in h2.GetName() or 'mtop' in h2.GetName() or 'width' in h2.GetName():
                continue
            #print ('name: ', h2.GetName(), '\t integral: ', h2.Integral())
            ...
            if (h2.GetName().find("HH") ==-1 or h2.GetName().find("hh") ==-1) and h2.GetName().find("data_obs") ==-1\
        ...
        zero_bincont += check_zerobincont(hSumi, nbins)
        ...
        fileOut.cd()
    return [errOcontSUMLast,errOcontSUMPLast, zero_bincont, bincontentLast]
