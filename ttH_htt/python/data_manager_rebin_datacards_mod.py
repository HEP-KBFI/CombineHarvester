...
def finMaxMin(histSource) :
    ...
    return 0
global check_zerobincont
def check_zerobincont(hist, nbin):
...
def getHist_tobeFlat(hist_dict, histSource, dataDY):
    for key in hist_dict.keys() :
        print hist_dict.keys()
        print 'key========', key, histSource
        if 'LBN_DY' in histSource:
            if dataDY:
                return hist_dict['data_DY']
            else:
                return hist_dict['DY']
        elif 'LBN_%s_' %key in histSource or 'LBN_GGF_%s' %key in histSource or 'LBN_VBF_%s' %key in histSource:
            print 'found key====== ', key
            return hist_dict[key]
        elif 'LBN_SingleTop' in histSource and 'ST' in key:
            return hist_dict['ST']

global rebinRegular_bbww
def rebinRegular_bbww(
    histSource,
    nbins,
    BINtype,
    do_signalFlat,
    ...
    print ("rebin")
    nonOther = ["TT", 'WJets', 'DY', 'data_DY', 'ST', 'data_obs', 'Fakes', 'fakes_mc']
    singleH_nobr = ['ttH', 'tHq', 'tHW', 'ggH', 'qqH', 'ZH', 'WH']
    singleH = []
    for H in singleH_nobr:
        for br in ['_hww', '_hzz', '_htt', '_hbb', '_hgg']:
            singleH.append(H+br)
    notconsider = ['hdamp', '_QCDbased', 'GluonMove', '_erdON', '_mtop', 'width', 'DY']
    if merge_Wjets: nonOther.remove('WJets')
    if merge_DY: nonOther.remove('DY')
    print ("bins: %s" % bin)
    ...
    hist_dict = {}
    for nkey, key in enumerate(file.GetListOfKeys()):
        obj =  key.ReadObj()
        if type(obj) is not TH1F:# and type(obj) is not ROOT.TH1D and type(obj) is not ROOT.TH1 and type(obj) is not ROOT.TH1S and type(obj) is not ROOT.TH1C :
            continue
        h2  = obj.Clone()
        if 'hdamp' in h2.GetName() or 'QCDbased' in h2.GetName() or 'GluonMove' in h2.GetName() or '_erdON' in h2.GetName() or 'mtop' in h2.GetName() or 'width' in h2.GetName():
            continue
        if dataDY and 'DY' in h2.GetName():
            if 'data' not in h2.GetName(): continue
        if h2.GetName().find('CMS') == -1 and h2.GetName().find('HH') == -1\
           and h2.GetName().find('hh') == -1 and h2.GetName().find('signal') == -1:
            if h2.GetName() not in nonOther and h2.GetName() not in singleH:
                if "Other" not in hist_dict.keys() :
                    print('Added in Other ' + h2.GetName())
                    hist_dict["Other"] = h2.Clone()
                    hist_dict["Other"].Sumw2()
                else:
                    print('Added in Other ' + h2.GetName())
                    hist_dict["Other"].Add(h2)
            elif h2.GetName() in singleH:
                if 'H' not in hist_dict.keys():
                     print('Added in H ' + h2.GetName())
                     hist_dict['H'] = h2.Clone()
                     hist_dict['H'].Sumw2()
                else:
                    print('Added in H ' + h2.GetName())
                    hist_dict['H'].Add(h2)
            elif h2.GetName() in ['TT', 'ST']:
                if 'TT' not in hist_dict.keys():
                     print('Added in TT ' + h2.GetName())
                     hist_dict['TT'] = h2.Clone()
                     hist_dict['TT'].Sumw2()
                else:
                    print('Added in TT ' + h2.GetName())
                    hist_dict['TT'].Add(h2)
                ...
                if dictkey not in hist_dict.keys():
                     print('Added in ' + dictkey)
                     hist_dict[dictkey] = h2.Clone()
                     hist_dict[dictkey].Sumw2()
                else:
                    print('Added in ' + dictkey)
                    hist_dict[dictkey].Add(h2)

        if signal_type == 'nonresNLO':
            if (h2.GetName().find("kl_") != -1 or h2.GetName().find('kt_') != -1) and h2.GetName().find("CMS") ==-1 and h2.GetName().find('dipole') == -1:
                print 'Added sig: ', h2.GetName()
            ...
            if h2.GetName().find("ggHH") != -1 and h2.GetName().find("CMS") == -1:
                print 'Added sig: ', h2.GetName()
                if not hSumSignal.Integral()>0 :
                    hSumSignal=h2.Clone()
                    hSumSignal.Sumw2()
            ...
            if h2.GetName().find("signal_ggf") != -1 and h2.GetName().find("CMS") == -1:
                print 'Added sig: ', h2.GetName()
                if not hSumSignal.Integral()>0 :
                    hSumSignal=h2.Clone()
                    hSumSignal.Sumw2()
        ...
        if (h2.GetName().find("HH") ==-1 or h2.GetName().find("hh") ==-1 or h2.GetName().find('signal') == -1) \
           and h2.GetName().find("data_obs") ==-1\
           and h2.GetName().find("fakes_mc") ==-1 and h2.GetName().find("CMS") ==-1:
            if not hSumAll.Integral()>0 :
                hSumAll=h2.Clone()
                hSumAll.Sumw2()
                hSumAll.SetName("hSumAllBkg")
            else :
                hSumAll.Add(h2)
        del h2
        #################end of loop over all hist############
    hist_dict["HH"] = hSumSignal
    if BINtype=="quantiles" :
        if do_signalFlat :
            if histSource.find('LBN') != -1:
                hist_tobeFlat = getHist_tobeFlat(hist_dict, histSource, dataDY)
                print ('hist tobeflat: ', hist_tobeFlat.GetName())
                nbinsQuant =  getQuantiles(hist_tobeFlat, nbins, 1.)
            else :
                nbinsQuant =  getQuantiles(hSumSignal, nbins, 1.)
        elif do_signalFlat_only:
            nbinsQuant =  getQuantiles(hist_dict["HH"], nbins, 1.)
        else  :
            nbinsQuant =  getQuantiles(hSumAll, nbins, 1.)
    for nn1, histogram in enumerate(histograms):
        nameHisto=histogram.GetName()
        if BINtype=="none" :
            ...
            histo.Sumw2()
        for place in range(0,histogram.GetNbinsX() + 1) :
            content =      histogram.GetBinContent(place)
            binErrorCopy = histogram.GetBinError(place);
            newbin =       histo.GetXaxis().FindBin(histogram.GetXaxis().GetBinCenter(place))
    ...
    if BINtype=="ranged" or BINtype=="regular":
        hSumi = TH1F( "hSumRebinned", "hSum" , nbins , 0., 1.)
    elif BINtype=="quantiles" : 
        hSumi = TH1F( "hSumRebinned", "hSum" , nbins , nbinsQuant)
    if not hSumi.GetSumw2N() : hSumi.Sumw2()
    for place in range(1,hSumAll.GetNbinsX()+1) :
        content=hSumAll.GetBinContent(place)
        newbin=hSumi.FindBin(hSumAll.GetBinCenter(place))
        binErrorCopy = hSumAll.GetBinError(place);
    ...
    for ibin in range(1, hSumi.GetNbinsX()+1):
        if hSumi.GetBinContent(ibin) <2:
            print('excluding ' + str(nbins) + 'as it contains less than two bincontent of bkg')
            return False, 0, 0
    if hSumi.GetBinContent(hSumi.GetNbinsX()) >0 :
        ratiohSum=hSumi.GetBinError(hSumi.GetNbinsX())/hSumi.GetBinContent(hSumi.GetNbinsX())
    errOcontSUMLast  = ratiohSum if ratiohSum<1.001 else 1.0
    bincontentLast   = hSumi.GetBinContent(hSumi.GetNbinsX())
    fileOut.cd()
    return [True, errOcontSUMLast, bincontentLast]
...
def rebinRegular(
    ...
    for nn,nbins in enumerate(nbin) :
        ...
        for nkey, keyF in enumerate(folders_Loop) :
            ...
            print 'loop_on======= ', file.GetListOfKeys()
...
def ReadLimits(bdtType, nbin, BINtype,channel,local,nstart,ntarget, sendToCondor, toAdd):
    ...
    up2=[]
    for nn,nbins in enumerate(nbin) :
        shapeVariable = "%s_%s%s" % (bdtType, nbins, toAdd)
...
global readLimit
def readLimit(datacardFile_output):
    f = open(datacardFile_output, 'r+')
    lines = f.readlines() # get all lines as a list (array)                                                                                
    for line in  lines:
        l = []
