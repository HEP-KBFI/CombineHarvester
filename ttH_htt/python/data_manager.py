...
def filter_genuineShape(inputShapesL, upDoNom):
    ...
    print tfilein, '\t', upDoNom[0], upDoNom[1], upDoNom[2]
    upInt = up.Integral()
    doInt = do.Integral()
    nomInt = nom.Integral()
    sum1 = 0
...
def make_minimumbincontent(hists):
    for hist in hists:
        for bin in range(1, hist.GetNbinsX()+1):
            if hist.GetBinContent(bin) ==0:
                hist.SetBinContent(bin, 0.00001*min(1, hist.Integral()))
                hist.SetBinError(bin, 0.00001*min(1, hist.Integral()))
...
def rebin_total(template, folder, fin, divideByBinWidth, name_total, dict) :
    ...
    print '************',total_hist.GetName(), '\t', folder, '\t', total_hist.GetBinError(1)
    hist = template.Clone()
