#!/usr/bin/env python
#python test/makePlots.py  --input  /home/snandan/hh_Analysis/CombineHarvestr/CMSSW_8_1_0/src/CombineHarvester/ttH_htt/dumb_bb2l_allBM//datacards_rebined//results/fitDiagnostics_shapes_combine_datacard_hh_bb2l_BDT_boosted_OS_MVAOutput_allBMs_quantiles_4bins_quantiles.root --odir /home/snandan/hh_Analysis/CombineHarvestr/CMSSW_8_1_0/src/CombineHarvester/ttH_htt/dumb_bb2l_allBM//datacards_rebined//plots/ --original /home/snandan/hh_Analysis/CombineHarvestr/CMSSW_8_1_0/src/CombineHarvester/ttH_htt/dumb_bb2l_allBM//datacards_rebined//datacard_hh_bb2l_BDT_boosted_OS_MVAOutput_allBMs_quantiles_4bins_quantiles.root --era 2016 --nameOut datacard_hh_bb2l_BDT_boosted_OS_MVAOutput_allBMs_quantiles_4bins_quantiles --channel 2l_0tau --HH --binToRead HH_2l_0tau --binToReadOriginal  HH_2l_0tau  --signal_type nonresNLO  --mass none  --HHtype bbWW  --unblind --do_bottom
...
if HH :
    ...
    if signal_type == 'nonresNLO':
        higgs_procs_to_draw = list(set(higgs_procs_to_draw) & set(['ggHH_kl_1_kt_1_hbbhww', 'ggHH_kl_1_kt_1_hbbhtt', 'ggHH_kl_1_kt_1_hbbhzz']))
    elif signal_type == 'res': 
        higgs_procs_to_draw = list(set(higgs_procs_to_draw) & set(['signal_ggf_%s_hbbhww' %mass, 'signal_ggf_%s_hbbhtt' %mass, 'signal_ggf_%s_hbbhzz' %mass]))
    else:
        higgs_procs_to_draw = list(set(higgs_procs_to_draw) & set(['ggHH_hbbhww', 'ggHH_hbbhtt', 'ggHH_hbbhzz']))
...
label_head += options.nameOut.replace('_final', '')
...
for cc, catcat in enumerate(catcats) :
    ...
    dataTGraph1, lastbin = rebin_data(
        template,
        dataTGraph1,
        readFrom,
        fin,
...
for cc, catcat in enumerate(catcats) :
    ...
    hist_total, lastbin = rebin_total(
        hist_total,
        readFrom,
        fin,
        divideByBinWidth,
...
if do_bottom :
    ...
    topPad = ROOT.TPad("topPad", "topPad", 0.00, 0.34, 1.00, 1.)
    ...
    bottomPad = ROOT.TPad("bottomPad", "bottomPad", 0.00, 0., 1.00, 0.34)
...
for key in dprocs.keys():
    for cc, catcat in enumerate(catcats) :
        if not options.fromHavester :
            ...
            readFrom = catcat
            readFrom += "_prefit"
        hist = fin[0].Get(readFrom + "/" + key )
        if hist:
           if key in higgs_procs_w_BR:
               if not find_SingleH:
                   singleH = hist
                   set_histproperty(singleH, dprocs[key], divideByBinWidth)
                   find_SingleH = True
                   continue
               else:
                   singleH.Add(hist)
                   continue
           set_histproperty(hist, dprocs[key], divideByBinWidth)
           hist_order.append(hist)
hist_order.append(singleH)
hist_order = sorted(hist_order, key=lambda x: x.Integral())
print hist_order
'''for hist in  hist_order:
    histogramStack_mc.Add(hist)'''
#for kk, key in  enumerate(dprocs.keys()) :
for kk, key in  enumerate(hist_order) :
    hist_rebin = key#template.Clone()
    print(hist_rebin.GetName(), '\t', hist_rebin.Integral())
    '''lastbin = 0
    addlegend = True
    print("Stacking ", key)
    for cc, catcat in enumerate(catcats) :
...
if do_bottom :
    ...
    for cc, catcat in enumerate(catcats) :
        ...
        hist_total_err, lastbin = do_hist_total_err(
            hist_total_err,
            labelX, histtotal  ,
